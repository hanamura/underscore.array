(function() {
  var dig, equal, oldArr, oldArray, pick, pickSome, rotate, same, select, _a,
    __slice = [].slice,
    _this = this;

  _a = {};

  _a.select = select = function(arr, key, def) {
  ...
  _a.pick = pick = function(arr) {
    return arr[Math.floor(arr.length * Math.random())];
  };

  _a.pickSome = pickSome = function(arr, len) {
    var a, i, index, _i, _results;
    a = [];
    _results = [];
    for (i = _i = 0; 0 <= len ? _i < len : _i > len; i = 0 <= len ? ++_i : --_i) {
      ...
      _results.push(a.splice(index, 1)[0]);
    }
    return _results;
  };
